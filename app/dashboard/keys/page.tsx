...
export default async function KeysManagementPage() {
  ...
  // Get all keys with vehicle information
  ...
  // Get ALL vehicles (allowing multiple keys per vehicle)
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Register and manage vehicle keys</p>
        </div>
        <Badge variant="info">{keys.length} Registered Keys</Badge>
      </div>
      ...
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Registered Keys</h2>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {currentTransaction && (
                      <div className="mt-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
                        <p className="text-xs font-semibold text-amber-800 mb-1">Checked out to:</p>
                        <p className="text-xs text-amber-700">{currentTransaction.user.fullName}</p>
                        <p className="text-xs text-amber-600">
                          Since: {new Date(currentTransaction.checkoutTime).toLocaleString('en-US')}
                        </p>
                      </div>
                    )}
      ...
      {keys.length === 0 && (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600">No keys registered</p>
          </CardContent>
        </Card>
      )}
    </div>
